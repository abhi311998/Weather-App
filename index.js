...
window.search_function = (event) => {
  ...
  var n_days = "3";
  const api = {
    key: "14ae1b19752d426a9e6160327202508", //"3f3acadf5f5f4c3daf9140532202308",
    base: "https://api.weatherapi.com/v1/"
  };

  const api_url= `${api.base}forecast.json?key=${api.key}&q=${query}&days=${n_days}`;
  ...
  xhttp.onreadystatechange = function(){
    ...
    var date = today.getDate() + '-' +(today.getMonth()+1) + '-' + today.getFullYear() + ' (' + day + ')';
    ...
    inner_div_0_heading.innerHTML = weatherData.location.name + ", " + weatherData.location.region + ", " + weatherData.location.country;
    ...
    max_min_temp.innerHTML = forecastday[0].day.maxtemp_c + " *C / " + forecastday[0].day.mintemp_c + " *C";

    // Second Section Part 2
    var today_condition_icon = document.getElementById("today_condition_icon");
    var fig_caption = document.getElementById("fig_caption");
    today_condition_icon.setAttribute("src", weatherData.current.condition.icon)
    ...
    precipitation.innerHTML = "Precipitation: " + weatherData.current.precip_mm + ' mm';
    wind_speed.innerHTML = "Wind Speed: " + weatherData.current.wind_mph + ' mph';
    pressure.innerHTML = "Pressure: " + weatherData.current.pressure_mb + ' mb';
    ...
    for (var i = 0; i < n_days; i++){
      ...
      if(i == 0){
        ...
        temp.appendChild(document.createTextNode(forecastday[i].day.avgtemp_c));
        cond.appendChild(document.createTextNode(forecastday[i].day.condition.text));
        humidity.appendChild(document.createTextNode(forecastday[i].day.avghumidity));
        ...
        wind_speed.appendChild(document.createTextNode(forecastday[i].day.maxwind_mph));
